...
def draw_image(image, results, labels, size):
    ...
    for idx, obj in enumerate(results):
        ...
        draw.text((ymin,xmin), display_str, font=ImageFont.truetype("AVHersheySimplexMedium.ttf", 20))
...
def make_interpreter(model_file, use_edgetpu):
    ...
    if use_edgetpu:
        ...
        return Interpreter(model_path=model_file, num_threads=multiprocessing.cpu_count())
...
def main():
    ...
    vs = VideoStream(0, usePiCamera=args.picamera, resolution=(640, 480))
    ...
    while True:
        try:
            ...
            image_pred = image.resize((input_width ,input_height), Image.Resampling.LANCZOS)
